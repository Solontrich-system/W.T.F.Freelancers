...
let allData     = { orders: [], transactions: [], gigs: [], users: [], chats: [] };
let charts      = {};
...
function renderOverviewChats(chats) {
  const el = document.getElementById('overview-chats');
  if (!el) return;
  if (!chats.length) { el.innerHTML = '<span class="empty-text">No conversations yet.</span>'; return; }
  el.innerHTML = chats.slice(0, 6).map(c => {
    const init  = initials(c.participant_name);
    return `<div class="chat-row">
      <div class="chat-row-left">
        <div class="mini-avatar">${init}</div>
        <div>
          <div class="chat-row-name">${c.participant_name || '—'}</div>
          <div class="chat-row-meta">${c.last_message || 'No messages'}</div>
        </div>
      </div>
      <div class="chat-row-right">
        <div>${fmtDate(c.last_message_at)}</div>
        ${c.unread_count > 0 ? `<div class="unread-pill">${c.unread_count}</div>` : ''}
      </div>
    </div>`;
  }).join('');
}
...
function renderMessagesPage() {
  const el  = document.getElementById('all-threads');
  const cnt = document.getElementById('all-chats-count');
  if (!el) return;
  if (cnt) cnt.textContent = `${allData.chats.length} total`;
  if (!allData.chats.length) { el.innerHTML = '<span class="empty-text">No conversations yet.</span>'; return; }
  el.innerHTML = allData.chats.map(c => {
    const init = initials(c.participant_name);
    return `<div class="thread-card">
      <div class="thread-avatar">${init}</div>
      <div class="thread-body">
        <div class="thread-name">${c.participant_name || '—'}</div>
        <div class="thread-preview">${c.last_message || 'No messages'}</div>
      </div>
      <div class="thread-right">
        <div class="thread-date">${fmtDate(c.last_message_at)}</div>
        ${c.unread_count > 0 ? `<div class="unread-pill">${c.unread_count} new</div>` : ''}
      </div>
    </div>`;
  }).join('');
}
...
async function loadAll() {
  ...
  try {
    const [users, gigs, orders, txns, chats] = await Promise.all([
      ...
      sbQuery('chat_threads', 'select=id,participant_name,last_message,last_message_at,unread_count&order=last_message_at.desc')
    ]);
    ...
    document.getElementById('kpi-chats').textContent   = fmt(chats.count);
    ...
    renderOverviewChats(chats.data);
