...
window.submitGig = async function() {
  if (!state.currentUser) return showToast('Please log in to post a gig.');
  ...
  const display  = state.currentProfile?.display_name || state.currentProfile?.full_name || state.currentUser.email.split('@')[0];
  ...
  if (error) { console.error(error); return showToast('Could not publish. Try again.'); }
...
// ── GIG DETAIL ────────────────────────────────────────────
// Uses state.currentGig and state.sellerPayDetails — no broken static exports
window.openGig = async function(id) {
  ...
  document.getElementById('detail-seller').textContent   = '👤 ' + (g.seller_name || 'Seller');
